...
{
    public class MappedGazePointsDataAggregator : UnfilteredOrLightlyFilteredGazeDataAggregator
    {
        public MappedGazePointsDataAggregator(ICurrentSessionData currentSessionData) : base(currentSessionData)
        {
        }
        ...
        protected override void AggregateInternal(IMonitoringContext monitoringContext, AggregatedData aggregatedData)
        {
            ...
            var fixationPoints = CurrentSessionData.GetGazePoints().ToList();
            var mappedFixationPoints = new List<MappedGazePoint>();
            ...
            using (var screenConfigurationsEnumerator = orderedScreenConfigurations.GetEnumerator())
            {
                ...
                foreach (var fixationPoint in fixationPoints)
                {
                    ...
                    if (gazePointDateTime >= monitoringContext.SubjectInfo.SessionStartTimestamp && gazePointDateTime <= screenConfigurationEndTime)
                    {
                        ...
                        var mappedFixationPoint = new MappedGazePoint()
                        {
                        ...
                        mappedFixationPoint.ScreenConfigurationName = screenConfiguration.Name;
                    }
            ...
            aggregatedData.MappedGazePoints = mappedFixationPoints;
        }
    }
}
